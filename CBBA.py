...
class CBBA_agent:
    def __init__(self, state, task, c_bar, task_index, id, vel, task_num, agent_num, L_t):
        # state为当前智能体位置，task为所有目标位置
        ...
        self.state = state  # Agent State (Position)
        self.task = task
    ...
    """
        ...
        while len(self.b) < self.L_t:  # 构建每个智能体的任务束
            ...
            # 计算时间折扣奖励
            if len(self.p) > 0:
                index = self.task_index.index(self.p[0])
                distance_j = 0
                distance_j += np.linalg.norm(self.state.squeeze() - self.task[index])  # 计算第一步距离
                S_p += (self.Lambda ** (distance_j / self.vel)) * self.c_bar[index]  # 计算第一步
                for p_idx in range(len(self.p) - 1):
                    index1=self.task_index.index(self.p[p_idx])
                    index1_=self.task_index.index(self.p[p_idx+1])
                    distance_j += np.linalg.norm(self.task[index1] - self.task[index1_])
                    S_p += (self.Lambda ** (distance_j / self.vel)) * self.c_bar[self.p[index1_]]

            # Calculate c_ij for each self.task j 计算任务得分
            best_pos = {}
            for j in J:
                c_list = []
                p_index = self.task_index.index(j)
                if j in self.b:  # If already in bundle list 以分配，再分配其他任务
                    ...
                    for n in range(len(self.p) + 1):
                        ...
                        distance_j += np.linalg.norm(state - task[p_index])  # 计算插入后的路径
                        # print(p_temp[0])
                        # print(self.c_bar)
                        # print(p_index)
                        # print(len(task))
                        # print(self.id)
                        c_temp += (self.Lambda ** (distance_j / self.vel)) * self.c_bar[p_index]
                        if len(p_temp) > 1:
                            for p_loc in range(len(p_temp) - 1):
                                distance_j += np.linalg.norm(self.task[p_temp[p_loc]] - self.task[p_temp[p_loc + 1]])
                    ...
                    c_j = c_list[max_idx]
                    self.c[p_index] = c_j
                    best_pos[p_index] = max_idx

            h = (self.c > self.y)  # 任务得分大于获胜报价，为1
            if sum(h) == 0:  # No valid self.task 没有有效的任务
                break
            self.c[~h] = 0  # 无效任务得分赋零
