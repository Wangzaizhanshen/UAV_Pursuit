...
class UAVPursuit(gym.Env):
    ...
    def __init__(self):
        ...
        self.target_r = 1
        self.tar_v_max = self.uav_v_max * 0.5
        self.tar_v_min = self.uav_v_min * 0.5
        ...
        self.obstacle_num = 5  # 障碍物数量
        self.obstacle_r_min = 20  # 障碍物最小半径
        self.obstacle_r_max = 30  # 障碍物最大半径
        self.obstacle = np.array([])
        # 敌方雷达探测区设置, 不可跨越
        self.radar_or_not = True
        self.radar_num = 3
        self.radar_d_min = 20
        self.radar_d_max = 40  # 雷达最大探测距离
        self.radar = np.array([])  # 存储雷达位置
        # 我机与雷达本时刻的距离和角度
        self.s_d = np.zeros([self.uav_num])
        self.s_a = np.zeros([self.uav_num])
        # 敌方火炮火力单元设置，设置为不可毁伤
        self.artillery_or_not = True
        self.artillery_num = 5
        self.artillery_attack_dis = 30  # 火炮最大攻击距离
        self.artillery_kill_dis = 20  # 火炮击杀距离
        self.artillery = np.array([])
        # 目标集结区设置
    ...
    def generate_circle(self, r_min, r_max, num, gather_x, gather_y, gather_r):
        ...
        if len(self.radar):
            for i in range(self.radar_num):
                circles.append(self.radar[i])
        if len(self.artillery):
            for i in range(self.artillery_num):
                circles.append(self.artillery[i])
        while True:
    ...
    def calc_probe_information(self, state):
        ...
        # 探测火炮禁飞区
        if self.artillery_or_not:
            for i in range(self.uav_num):
                # 计算每个无人机到每个障碍物的距离和角度
                obs_d = -np.ones([self.artillery_num, ])
                obs_a = np.zeros([self.artillery_num, ])
                for j in range(self.artillery_num):
                    obs_d[j] = self.calc_dis(state[i][0], state[i][1], self.artillery[j][0],
                                             self.artillery[j][1]) - \
                               self.artillery[j][2]
                    obs_a[j] = self.calc_angle(state[i][0], state[i][1], state[i][2], state[i][3],
                                               self.artillery[j][0], self.artillery[j][1])
                    # 判断该障碍物不在无人机探测范围内
                    if obs_d[j] > self.uav_probe or abs(obs_a[j]) > np.degrees(self.uav_probe_theta):
                        continue
                    # 判断该障碍物到无人机的距离是否在最近的5个中
                    temp_list_d = [state[i][6], state[i][8], state[i][10], state[i][12], state[i][14]]
                    temp_list_d, insert_i = self.insert_number(temp_list_d, obs_d[j])
        ...
        # 探测雷达禁飞区
        if self.radar_or_not:
            for i in range(self.uav_num):
                # 计算每个无人机到每个障碍物的距离和角度
                obs_d = -np.ones([self.radar_num, ])
                obs_a = np.zeros([self.radar_num, ])
                for j in range(self.radar_num):
                    obs_d[j] = self.calc_dis(state[i][0], state[i][1], self.radar[j][0],
                                             self.radar[j][1]) - self.radar[j][2]
                    obs_a[j] = self.calc_angle(state[i][0], state[i][1], state[i][2], state[i][3],
                                               self.radar[j][0], self.radar[j][1])
                    # 判断该障碍物不在无人机探测范围内
                    if obs_d[j] > self.uav_probe or abs(obs_a[j]) > np.degrees(self.uav_probe_theta):
                        continue
                    # 判断该障碍物到无人机的距离是否在最近的5个中
    ...
    def calc_t_probe_information(self, t_state, state):
        ...
        if self.artillery_or_not:
            for i in range(self.target_num):
                ...
                obs_a = np.zeros([self.artillery_num, ])
                for j in range(self.artillery_num):
                    obs_d[j] = self.calc_dis(t_state[i][0], t_state[i][1], self.artillery[j][0],
                                             self.artillery[j][1]) - \
                               self.artillery[j][2]
                    obs_a[j] = self.calc_angle(t_state[i][0], t_state[i][1], t_state[i][2], t_state[i][3],
                                               self.artillery[j][0], self.artillery[j][1])
        ...
        if self.radar_or_not:
            for i in range(self.target_num):
                ...
                obs_a = np.zeros([self.radar_num, ])
                for j in range(self.radar_num):
                    obs_d[j] = self.calc_dis(t_state[i][0], t_state[i][1], self.radar[j][0],
                                             self.radar[j][1]) - self.radar[j][2]
                    obs_a[j] = self.calc_angle(t_state[i][0], t_state[i][1], t_state[i][2], t_state[i][3],
                                               self.radar[j][0], self.radar[j][1])
                    # 判断该障碍物不在无人机探测范围内
    ...
    def generate_uav(self):
        while True:
            ...
            if self.area == 1:  # 左
                state[0] = random.uniform(10, 0.2 * self.width)
                state[1] = random.uniform(10, 0.3 * self.height)
            elif self.area == 2:  # 中
                state[0] = random.uniform(0.2 * self.width, 0.8 * self.width)
                state[1] = random.uniform(0.3 * self.height, 0.7 * self.height)
            elif self.area == 3:  # 右
                state[0] = random.uniform(0.8 * self.width, 490)
                state[1] = random.uniform(0.8 * self.height, 490)
                state[3] = math.pi
            # 集结区
            ...
            if self.artillery_or_not:
                for i in range(self.artillery_num):
                    temp_d = self.calc_dis(state[0], state[1], self.artillery[i][0], self.artillery[i][1])
                    if temp_d > self.uav_r + self.artillery[i][2] + 20:
                        done += 1
            if self.radar_or_not:
                for i in range(self.radar_num):
                    temp_d = self.calc_dis(state[0], state[1], self.radar[i][0], self.radar[i][1])
                    if temp_d > self.uav_r + self.radar[i][2] + 20:
                        done += 1
            if done == self.obstacle_num + self.artillery_num + self.radar_num:
                return state
    ...
    def generate_t_uav(self):
        while True:
            ...
            t_state[0] = random.uniform(10, 0.2 * self.width)
            t_state[1] = random.uniform(0.7 * self.height, 490)
            # 集结区
            ...
            if self.artillery_or_not:
                for i in range(self.artillery_num):
                    temp_d = self.calc_dis(t_state[0], t_state[1], self.artillery[i][0], self.artillery[i][1])
                    if temp_d > self.uav_r + self.artillery[i][2] + 20:
                        done += 1
            if self.radar_or_not:
                for i in range(self.radar_num):
                    temp_d = self.calc_dis(t_state[0], t_state[1], self.radar[i][0], self.radar[i][1])
                    if temp_d > self.uav_r + self.radar[i][2] + 20:
                        done += 1
            if done == self.obstacle_num + self.artillery_num + self.radar_num:
                return t_state
    ...
    def situation_assessment(self, state, t_state):
        ...
        row_mapping = np.where(non_zero_rows)[0]  # 存活的敌方无人机索引
        col_mapping = np.where(non_zero_cols)[0]  # 存活的我方无人机索引
        s_threat_0 = s_threat_.copy()

        # 进行迭代分配
        # 当我方无人机数量大于等于目标数量，进行一对一分配：
        while True:
            if len(col_mapping) != 0:  # 仍存在目标
                if len(row_mapping) > 1:
                    # if len(row_mapping) >= len(col_mapping):
                    # 初始化CBBA算法，进行目标分配
                    self.task = []
                    self.task_index = []
                    for i in range(len(col_mapping)):
                        self.task_index.append(col_mapping[i])
                        self.task.append([t_state[col_mapping[i]][0], t_state[col_mapping[i]][1]])
                    self.agent = [
                        CBBA_agent([state[row_mapping[i]][0], state[row_mapping[i]][1]], self.task, s_threat_[i],
                                   self.task_index, i,
                                   state[row_mapping[i]][2], len(col_mapping), len(row_mapping), 1) for
                        i in range(len(row_mapping))]
                    # for i in range(len(row_mapping)):
                    #     print(len(self.agent[i].c_bar))
                    self.G = np.ones((len(row_mapping), len(row_mapping)))
                    self.assignment_result = self.target_assignment(len(row_mapping), self.task_index)
                    # print(self.assignment_result)
                    # 处理分配信息
                    uav_ind = row_mapping.tolist()  # 存活敌机ID
                    tar_ind = [-1 for _ in range(len(row_mapping))]
                    for i in range(len(uav_ind)):
                        # print(self.assignment_result[i])
                        if self.assignment_result[i] != -1:
                            tar_ind[i] = self.assignment_result[i]
                elif len(row_mapping) == 1:
                    ind = np.argmax(s_threat_[0])
                    tar_ind = [col_mapping[ind]]
            ...
            #     for i in range(len(col_mapping)):
            #         if i in ind:
            #             task.append(col_mapping[i])
            #         else:
            #             d.append(i)
            #     s_threat_1 = np.delete(s_threat_, d, axis=1)
            #     if num == 1:
            #         uav_ind = row_mapping.tolist()
            #         tar_ind = [col_mapping[sort_ind[-1]]]
            #     else:
            #         # 重新初始化CBBA信息
            #         self.task = []
            #         self.task_index = []
            #         # print(len(col_mapping))
            #         for i in range(len(task)):
            #             self.task_index.append(task[i])
            #             self.task.append([t_state[task[i]][0], t_state[task[i]][1]])
            #         self.agent = [
            #             CBBA_agent([state[row_mapping[i]][0], state[row_mapping[i]][1]], self.task, s_threat_1[i],
            #                        self.task_index, i,
            #                        state[row_mapping[i]][2], len(col_mapping), len(row_mapping), 1) for
            #             i in range(len(row_mapping))]
            ...
            #         # 进行目标分配
            #         self.assignment_result = self.target_assignment(num, task)
            #         # print(self.assignment_result)
            #         # 处理分配信息
            #         uav_ind = row_mapping.tolist()
    ...
    # 进行目标分配
    def target_assignment(self, num, task):
        ...
        while True:
            ...
            message_pool = [robot.send_message() for robot in self.agent]

            for robot_id, robot in enumerate(self.agent):
            ...
            if sum(converged_list) == num:
                for robot in self.agent:
                    if len(robot.p):
                        assignment_result.append(task[robot.p[0]])
                    else:
                        assignment_result.append(-1)
                break
            # else:
            #     for robot in self.agent:
            #         if len(robot.p):
    ...
    def reset(self):
        ...
        # 生成雷达探测区
        if self.radar_or_not:
            self.radar = self.generate_circle(self.radar_d_min, self.radar_d_max, self.radar_num,
                                              self.gather_x, self.gather_y, self.gather_r)
        # 生成火炮区
        if self.artillery_or_not:
            self.artillery = self.generate_circle(self.obstacle_r_min, self.obstacle_r_max, self.artillery_num,
                                                  self.gather_x, self.gather_y, self.gather_r)
        # 初始化敌方巡飞弹
        t_state = []
        for target in range(self.target_num):
    ...
    def step(self, action, t_action):  # 输入的action时归一化后的
        ...
        self.situation_assessment(self.state_, self.t_state_)
        uav_id = self.uav_ind.copy()
        tar_id = self.tar_ind.copy()
        # print("uav_id:",uav_id)
        # print("tar_id:", tar_id)
    ...
    def render(self, mode='human'):
        ...
        if self.artillery_or_not:
            # 设置渐变颜色
            start_color = (255, 0, 0)  # 开始颜色为红色
            end_color = (255, 255, 0)  # 结束颜色为橙色
            for circle in self.artillery:
                for i in range(circle[2], 0, -7):
                    color = (
                        int(start_color[0] + (end_color[0] - start_color[0]) * (i / circle[2])),
                        int(start_color[1] + (end_color[1] - start_color[1]) * (i / circle[2])),
                        int(start_color[2] + (end_color[2] - start_color[2]) * (i / circle[2]))
                    )
                    pygame.draw.circle(self.surf, color, (circle[0], circle[1]), i)
        # 画出雷达禁飞区
        if self.radar_or_not:
            # 设置渐变颜色
            start_color = (0, 0, 255)  # 开始颜色为蓝色
            end_color = (158, 189, 25)  # 结束颜色为绿色
            k = 0
            for circle in self.radar:
                for i in range(circle[2], 0, -7):
                    color = (
                        int(start_color[0] + (end_color[0] - start_color[0]) * (i / circle[2])),
                        int(start_color[1] + (end_color[1] - start_color[1]) * (i / circle[2])),
    ...
    def draw(self, episode):
        ...
        if self.artillery_or_not:
            ...
            for circle in self.artillery:
                for i in range(circle[2], 0, -7):
                    col = (
                        (start_color[0] + (end_color[0] - start_color[0]) * (i / circle[2])) / 255,
                        (start_color[1] + (end_color[1] - start_color[1]) * (i / circle[2])) / 255,
                        (start_color[2] + (end_color[2] - start_color[2]) * (i / circle[2])) / 255
                    )
                    obstacle = Circle((circle[0], circle[1]), i, color=col)
                    ax.add_patch(obstacle)
        # 雷达禁飞区
        if self.radar_or_not:
            # 设置渐变颜色
            ...
            for circle in self.radar:
                for i in range(circle[2], 0, -7):
                    ...
                    radar = Circle((circle[0], circle[1]), i, color=col)
                    ax.add_patch(radar)
                k += 1
        # 我方巡飞弹轨迹
        self.state = self.state_normalization(self.state, False)
        for i in range(self.uav_num):
            ...
            ax.add_patch(agent_points)
        self.state = self.state_normalization(self.state, True)
        # plt.savefig('D:/code/UAV_Penetration/UAV_Penetration/MFDDPG/图{}.png'.format(episode), dpi=300)
        plt.show()
    ...
    def draw_gif(self, episode):
        ...
        ani = FuncAnimation(fig, update_lines, frames=frame, fargs=(data, lines, lines1, points, points1),
                            interval=20)
        # ani.save('D:/code/UAV_Penetration/UAV_Penetration/MFDDPG/动图{}.gif'.format(episode), writer='pillow',
        #          fps=1000)
        plt.show()
