            ...
            if len(col_mapping) != 0:  # 仍存在目标
                if len(row_mapping) > 1:
                    ...
                    for i in range(len(col_mapping)):
                        self.task.append(col_mapping[i])
                        self.tar_state.append([t_state[col_mapping[i]][0], t_state[col_mapping[i]][1]])
                    for i in range(len(row_mapping)):
                        self.agent_state.append([state[row_mapping[i]][0], state[row_mapping[i]][1]])
                    self.agent = [
                        CBBA_agent(self.agent_state, self.tar_state, s_threat_[i], i,
                    ...
                    agent_id, tar_id = self.target_assignment(len(row_mapping))
    ...
    def target_assignment(self, num):
        ...
        while True:
            ...
            if sum(converged_list) == num:
                for robot in self.agent:
                    if len(robot.p):
                        tar_id.append(robot.p[0])
                    else:
                        tar_id.append(-1)
                break

        return agent_id, tar_id
